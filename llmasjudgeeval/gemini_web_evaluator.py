"""
LLM-as-a-Judge Evaluation System for Gemini with Web Search
Evaluates Gemini (with Google Search) responses against ground truth using LLM judgment
"""
import os
import sys
import json
...
# Reuse the LLMJudge class from the existing evaluator if possible, 
# but to keep this script standalone and avoid import issues if paths change, 
# I will include the LLMJudge class here as well.
...
class GeminiWebClient:
    """Client for Gemini with Google Search enabled"""
    ...
    def process_query(self, query: str) -> Dict:
        """
        Process a query using Gemini with Web Search
        Returns a dict compatible with the evaluator's expectation
        """
        try:
            # Add a system prompt or context to guide the model to be a legal assistant
            prompt = f"""You are a helpful legal assistant. Answer the following question accurately.
...
"""
            ...
            # Mock routing info for compatibility
            return {
                'final_answer': final_answer,
                'routing': {
                    'systems': ['gemini_web_search']
                }
            }
        except Exception as e:
...
class EvaluationRunner:
    ...
    def __init__(
        ...
        judge: LLMJudge,
        output_dir: str = "gemini_web_evaluation_results"
    ):
        self.client = client
        self.judge = judge
    ...
    def run_evaluation(
        ...
        print("=" * 80)
        print("LLM-as-a-Judge Evaluation for Gemini 2.5 Flash (Google Search Grounding)")
        print("=" * 80)
        ...
        for idx, item in enumerate(ground_truth, 1):
            ...
            try:
                # Get answer from Gemini Client
                print("  → Querying Gemini with Web Search...")
                query_start = time.time()
                response = self.client.process_query(question)
                query_time = time.time() - query_start
                
                generated_answer = response.get('final_answer', '')
                systems_used = response.get('routing', {}).get('systems', [])
    ...
    def _generate_summary_report(self, evaluation_output: Dict):
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            f.write("Gemini 2.5 Flash (Google Search Grounding) Evaluation\n")
            f.write("=" * 80 + "\n\n")
...
def main():
    ...
    output_dir = script_dir / "gemini_web_evaluation_results"
    ...
    print("\nInitializing Gemini Web Client...")
    client = GeminiWebClient(api_key=GEMINI_API_KEY, model_name="gemini-2.5-flash")
