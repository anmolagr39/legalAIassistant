"""
LLM-as-a-Judge Evaluation System for Simple Gemini 2.5 Flash (No Web Search)
Evaluates plain Gemini responses against ground truth using LLM judgment
"""
import os
import sys
import json
...
class GeminiSimpleClient:
    """Simple Gemini 2.5 Flash client without web search"""
    ...
    def query(self, question: str) -> str:
        """
        Query Gemini with a simple prompt (no web search)
        ...
        """
        try:
            prompt = f"""You are a legal expert assistant specializing in Indian law. Answer the following question accurately and comprehensively based on your knowledge.
...
class EvaluationRunner:
    ...
    def __init__(
        self,
        gemini_client: GeminiSimpleClient,
        judge: LLMJudge,
        output_dir: str = "gemini_simple_evaluation_results"
    ):
    ...
    def run_evaluation(
        ...
        print("=" * 80)
        print("LLM-as-a-Judge Evaluation for Gemini 2.5 Flash (Simple - No Web Search)")
        print("=" * 80)
        ...
        for idx, item in enumerate(ground_truth, 1):
            ...
            try:
                # Get answer from Gemini (no web search)
                print("  → Querying Gemini 2.5 Flash (simple)...")
                query_start = time.time()
                generated_answer = self.gemini_client.query(question)
    ...
    def _generate_summary_report(self, evaluation_output: Dict):
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            f.write("Gemini 2.5 Flash (Simple - No Web Search)\n")
            f.write("=" * 80 + "\n\n")
...
def main():
    ...
    output_dir = script_dir / "gemini_simple_evaluation_results"
    ...
    print("\nInitializing Gemini 2.5 Flash Client (Simple - No Web Search)...")
    gemini_client = GeminiSimpleClient(api_key=GEMINI_API_KEY, model="gemini-2.5-flash")
